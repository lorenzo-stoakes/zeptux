...
static uint32_t lapic_read_reg(uint64_t offset)
{
	return *lapic_reg_ptr(offset);
}

// Write to local APIC register.
static void lapic_write_reg(uint64_t offset, uint32_t val)
{
...
// Clear error status.
static void lapic_clear_error(void)
{
	// Requires 2 writes.
	lapic_write_reg(APIC_ESR_OFFSET, 0);
	lapic_write_reg(APIC_ESR_OFFSET, 0);
}

// Mark an interrupt cleared.
static void lapic_clear_interrupt(void)
{
	lapic_write_reg(APIC_EOI_OFFSET, 0);
}
...
static void lapic_init(void)
{
	...
	// 3a, figure 10-7.
	uint32_t ver = lapic_read_reg(APIC_VER_OFFSET);
	...
	lapic_write_reg(APIC_LINT0_OFFSET, APIC_MASK_FLAG);
	lapic_write_reg(APIC_LINT1_OFFSET, APIC_MASK_FLAG);

	// Clear any error status.
	lapic_clear_error();
	// Clear any residual interrupts.
	lapic_clear_interrupt();

	// Set error interrupt IRQ.
	lapic_write_reg(APIC_ERROR_OFFSET, APIC_IRQ_0 + APIC_IRQ_ERROR);

	// Configure the timer.
	lapic_write_reg(APIC_TDCR_OFFSET, 0xb); // Divide counts by 1.
	lapic_write_reg(APIC_TIMER_OFFSET,
			APIC_PERIODIC_FLAG | (APIC_IRQ_0 + APIC_IRQ_TIMER));
	// We count down from this.
	lapic_write_reg(APIC_TICR_OFFSET, APIC_INIT_TIMER_COUNT_VALUE);

	// Disable performance counter overflow interrupts.
	if (num_lvt_entries >= 4)
		lapic_write_reg(APIC_PCINT_OFFSET, APIC_MASK_FLAG);

	// Enable local APIC (step 1) - specify spurious interrupt.
	// See https://en.wikipedia.org/wiki/Interrupt#Spurious_interrupts
	lapic_write_reg(APIC_SPURIOUS_OFFSET,
	...
	lapic_write_reg(APIC_TPR_OFFSET, 0);
}
